...
void Vec4::prnt(void) 
//------------------------------------------------------------------------------
// Unlabeled prettyprint of Vec3 elements.
{
...
VECTYPE tmp    = (VECTYPE)0.0;
 ...
 //   DPF("Vec4:doublenorm");
	...
	{
#if(DIAGNOSTICS==TRUE)
		DPF("\n ???Vec4::norm() given 0-length src vector!!!");
#endif
		return((VECTYPE)0.0);
	}
}
...
Vec4 out;
    ...
    for(i=0; i<VV; i++) 
	{
		out.row[i] = src1.row[i] + src2.row[i];
	}
    *this = out;	
}
...
//------------------------------------------------------------------------------
// Multiply by scalar: [this]= src1 + src2
{
int i;
Vec4 out;
    ...
    for(i=0; i<VV; i++) 
	{
		out.row[i] = src1.row[i]*c;
	}
    *this = out;	
}

VECTYPE Vec4::dot(Vec4& src1, Vec4& src2)
//------------------------------------------------------------------------------
...
void Vec4::cross(Vec4& src1, Vec4& src2) 
//------------------------------------------------------------------------------
// Cross product of the 1st 3 terms of two 4-element vectors that have 
// been normalized by their 4th element (i.e. homogeneous coords used).
{
VECTYPE norm;
Vec4 out;

    if(src1.row[3] == (VECTYPE)0.0) 
	{
		DPF("\n!!!Vec4::cross(): src1.row[3]=0.0, so NOP!!!\n");
		return;
	}
    if(src2.row[3] == (VECTYPE)0.0) 
	{
		DPF("\n!!!Vec4::cross(): src2->row[3]=0.0, so NOP!!!\n");
		return;
	}
    if(src1.row[3] != (VECTYPE)1.0) 
	{
#if(DIAGNOSTICS==TRUE)
		DPF("\n!!WARNING!! Vec4::cross(): src1 norm'd by src1.row[3]!!!\n");
#endif
		norm = (VECTYPE)1.0 / src1.row[3];
		src1.row[0] *= norm;
		src1.row[1] *= norm;
		src1.row[2] *= norm;
		src1.row[4] = (VECTYPE)1.0;
	}
    if(src2.row[3] != (VECTYPE)1.0) 
	{
#if(DIAGNOSTICS==TRUE)
		DPF("\n!!WARNING!! Vec4::cross(): src2 norm'd by src2.row[3]!!!\n");
#endif
		norm = (VECTYPE)1.0 / src2.row[3];
		src2.row[0] *= norm;
		src2.row[1] *= norm;
		src2.row[2] *= norm;
		src2.row[3] = (VECTYPE)1.0;
	}
    out.row[0] = src1.row[1]* src2.row[2] - src1.row[2]* src2.row[1];
    out.row[1] = src1.row[2]* src2.row[0] - src1.row[0]* src2.row[2];
    out.row[2] = src1.row[0]* src2.row[1] - src1.row[1]* src2.row[0];
    out.row[3] = (VECTYPE)1.0;
    *this = out;
}
...
VECTYPE nx2,ny2,nz2;				// squared terms.
    ...
    axlong = normaxis.norm(axis);	// get a normalized axis;
...
// destructor; de-alloc memory before we destroy the object.
{
    if(a != NULL) 
	{
    	delete[xsize*ysize] a;		// release memory, if any
    	a = NULL;					// (for VC++)
    } 
#if (PESKYWARNINGS==TRUE)
    else 
...
// return FALSE, else return TRUE.
{
    ...
    delete[xsize*ysize] a;			// release all pixel memory.
	a = NULL;						// (for VC++)
    xsize = 0;
	ysize = 0;
    return(TRUE);
...
// return FALSE, else return TRUE.
{
    ...
    delete[size] a;					// release all pixel memory.
	a = NULL;						// (for VC++)
    size = 0;
    return(TRUE);
}
