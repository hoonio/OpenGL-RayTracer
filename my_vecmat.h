...
arguments to the function)never causes problems.  The results of the calculations
of each member function places results in the data members of the calling 
object, unless the result is of a different type; then the member function's 
results are passed as a returned value.  
...
01/23/01	J. Tumblin more for CShape library work (Comp. Geometry)
01/22/05	J. Tumblin took useful subset to make 'my_vecmat' for my students.
==============================================================================*/
...
/* an 3-D homogeneous 'column vector';
	...
	VECTYPE norm(Vec4& src); 			// normalize, then return length
	...
	VECTYPE dot(Vec4& src1, Vec4& src2);// dot product; retn [in1].[in2]
	void cross(Vec4& src1, Vec4& src2);	// homogeneous coord cross prod;
										// [this]=[src1]X[src2]
...
//		   [   ...         ...			 ...	   ... ]
	...
	void mmpy(Mat4& left, Mat4& rt);	// matrix-matrix mpy:
										// [this] = [this][rt]
	void transpose(void);				// in-place transpose
...
public:	
	...
	BOOL SVD_full(Matrx& u, Matrx& ss, Matrx& v);
									...
									// Perform a full Singular Value Decomposition
